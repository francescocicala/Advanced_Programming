...
int main() {
  ...
  int* ap[7]; // vector of int pointers
  ...
  pv = ppc; // pv can contain any pointer, since it is void type
  pv = ap;
  pv = pi;
  ...
  // strings are a special type of char[]
  char word[]{"Hello"};
  char word2[]{'H', 'e', 'l', 'l', 'o', 0};
  if (strcmp(word, word2) == 0) // word == word2 is implementationt-dependent
                                 // it might compare the two pointers)
    std::cout << "same\n";
  else
    std::cout << "different\n";

  int (*fp)(const char*); // pointer to func 1
  fp = func1;
  ...
  std::cout << "7777: " << sizeof(x) << std::endl;
